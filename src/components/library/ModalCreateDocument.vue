<template>
    ...
    <v-dialog v-model="dialog" width="500">
        <v-card>
            <v-toolbar>
                <span class="px-4 w-full text-center text-cyan-500 font-bold title_views">AGREGAR DOCUMENTO</span>
            </v-toolbar>
            <v-card-text>
                <v-col cols="12">
                    ...
                    <v-select label="Curso relacionado" variant="outlined" :items=dataCoursesDescription
                        color="cyan-lighten-1" v-model="course"></v-select>
...
export default {
    ...
    setup(_, { emit }) {
        ...
        const dataCoursesDescription = ref([]);
        const files = ref([]);
        ...
        const readyData = async () => {
            findAllCoursesHomeApi(store.state.token)
                .then(response => {
                    ...
                    dataCoursesDescription.value = dataCourses.value.map(course => course.title)
                })
                .catch(error => {
        ...
        const createItem = () => {
            const findCourse = dataCourses.value.find(courseItem => courseItem.title == course.value)
            emit('create-item', {
                document_title: title.value,
                document_description: description.value,
                course_id: findCourse.course_id,
                document: files.value[0]

            })
            title.value = description.value = course.value = "";
            dataCourses.value = [];
            dataCoursesDescription.value = [];
            dialog.value = false
        }
